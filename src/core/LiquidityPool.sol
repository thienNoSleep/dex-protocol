 ...
 * @title LiquidityPool
 *      @author ThienLe
 *y=k) AMM pool for an arbitrary ERC20 pair. Mints LP tokens to providers; 0.3% swap fee accrues to reserves.
 */
...
contract LiquidityPool is ERC20, ReentrancyGuard {
    ...
    event LiquidityAdded(address indexed provider, uint256 amountA, uint256 amountB, uint256 liquidityTokenAmount);
    event LiquidityRemoved(address indexed Cashouted, uint256 amountA, uint256 amountB, uint256 lpBurn);
     ...
     */
    function removeLiquidity(uint256 liquidityTokenAmount) public nonReentrant {
        require(liquidityTokenAmount > 0, InvalidAmount());
        uint256 amountA = liquidityTokenAmount * reserveA / totalSupply();
        uint256 amountB = liquidityTokenAmount * reserveB / totalSupply();
    ...
    function swap(uint256 amountIn, IERC20 _token0) public nonReentrant {
        ...
        uint256 amountInWithFee = amountIn * 997 / 1000; //0.3%
